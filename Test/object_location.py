...
class ImageComparator:
    ...
    def __init__(self, device="cpu"):
        """
        Initializes the ImageComparator by loading the CLIP model and setting the device.
    ...
    def compare_images(self, image_path1, image_path2):
        ...
        try:
            ...
            logging.error(f"Error comparing images {image_path1} and {image_path2}: {e}")
            return False
...
class DatabaseHandler:
    ...
    def __init__(self, db_path):
        """
        Initializes the DatabaseHandler by setting up the database connection.
    ...
    def update_object_coordinates(self, row_id, obj_latitude, obj_longitude, obj_number):
        ...
        try:
            ...
            logging.error(f"Database update error for row ID {row_id}: {e}")
...
def calculate_coordinates(lat1, lon1, lat2, lon2, angle1, angle2):
    ...
    try:
        ...
        logging.error(f"Error calculating coordinates: {e}")
        raise


def process_images_and_update_db(db_handler, image_comparator,directory):
    """
    ...
    try:
        ...
        for i in range(len(entries)):
            ...
            if angle1 is None:
                logging.error(f"Error: Camera angle not found for ID {id1}")
                continue
            ...
            for j in range(i + 1, len(entries)):
                ...
                if camera_id1 != camera_id2 and id2 in unprocessed_ids:
                    ...
                    if angle2 is None:
                        logging.error(f"Error: Camera angle not found for ID {id2}")
                        continue

                    # Compare images
                    image_path1 = os.path.join(directory, "Captured", f"{id1}.jpg")
                    image_path2 = os.path.join(directory, "Captured", f"{id2}.jpg")

                    if image_comparator.compare_images(image_path1, image_path2):
                        logging.info(f"Images {id1} and {id2} are similar. Processing...")
                        ...
                        try:
                            ...
                            logging.info(f"Updated object coordinates in database for {id1} and {id2}.")
                            ...
                            logging.error(f"Error calculating coordinates for IDs {id1} and {id2}: {e}")
...
if __name__ == "__main__":
    ...
    process_images_and_update_db(db_handler, image_comparator,directory)
